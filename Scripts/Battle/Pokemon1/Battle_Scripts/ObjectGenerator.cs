...
public class ObjectGenerator : MonoBehaviour
{
    ...
    public GameObject pokemonMenuBoxPlefab;


    // サポート用変数
    public BattleManager battleManager; // battleManagerオブジェクトのBattleManagerスクリプトを取得
    public Trainer_Script Trainer; // TrainerオブジェクトのTrainer_Scriptスクリプトを取得
    ...
    public GameObject pokemonMenuBoxObj;
    ...
    public void Generate_Menu()
    {
        ...
        foreach (Transform ob in children)
        {
            ...
            // ポケモンボタン
            if (ob.name == "PokemonButton_UI")
            {
                Button pokemonButton = ob.GetComponent<Button>();
                // OnPokemonButton関数を入れる
                pokemonButton.onClick.AddListener(() => OnPokemonButton());
            }

            // にげるボタン
            if (ob.name == "EscapeButton_UI")
            {
                Button escapeButton = ob.GetComponent<Button>();
                // OnEscapeButton関数を入れる
    ...
    void OnBattleButton()
    {
        ...
        if (battleManager.canPlayerSelect)
        {
            ...
            // ポケモンメニューが残っているなら
            if (pokemonMenuBoxObj)
            {
                // ポケモンメニュー　削除
                Destroy(pokemonMenuBoxObj);
            }
    ...
    void OnPokemonButton()
    {
        ...
        if (battleManager.canPlayerSelect)
        {
            ...
            if (!pokemonMenuBoxObj)
            {
                Generate_PokemonMenu();
            }
        }
    }

    void Generate_PokemonMenu()
    {
        // メニューボックス　作成
        pokemonMenuBoxObj = Instantiate(pokemonMenuBoxPlefab);
        // キャンバスの子オブジェクトとして追加
        pokemonMenuBoxObj.transform.SetParent(canvas.transform, false);
    }
